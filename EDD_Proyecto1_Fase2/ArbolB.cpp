...
ArbolB::~ArbolB() {
    // pendiente: limpieza de memoria post-order
}
...
// wrapper publico para la busqueda
...
void ArbolB::dividirHijo(NodoB* padre, int indice, NodoB* hijo_lleno) {
    // creamos el nuevo nodo z que sera el hermano derecho de y
    ...
    // si no es hoja, tambien le pasamos los ultimos 3 punteros de los hijos
    ...
    // el hijo lleno original se queda solo con las primeras 3 claves
...
void ArbolB::insertarNoLleno(NodoB* nodo, string fecha, string cod_lote, string cod_finca, string nom_finca, int sacos, string tipo) {
    ...
    // primero verificamos si la fecha ya existe en este nodo
    ...
    if (nodo->es_hoja) {
        ...
        // verificamos si la fecha ya esta en algun hijo o descendiente
        // este pre-chequeo evita hacer splits innecesarios si la fecha ya existia
...
void ArbolB::insertarLote(string fecha, string cod_lote, string cod_finca, string nom_finca, int sacos, string tipo) {
    if (raiz == nullptr) {
        // el arbol esta vacio, creamos la raiz
        raiz = new NodoB(true);
        ...
        // si la raiz esta completamente llena (6 claves)
        if (raiz->cantidad_claves == 6) {
            // creamos una nueva raiz vacia
            NodoB* nueva_raiz = new NodoB(false);
            nueva_raiz->hijos[0] = raiz;
            
            // dividimos la antigua raiz
            dividirHijo(nueva_raiz, 0, raiz);
            
            // insertamos el dato nuevo
            int i = 0;
            ...
            // actualizamos el puntero maestro
            raiz = nueva_raiz;
        } else {
            // si la raiz no esta llena, insertamos normal
            insertarNoLleno(raiz, fecha, cod_lote, cod_finca, nom_finca, sacos, tipo);
        }
    }
}

// recorrido para listar todas las fechas
...
// wrapper publico para mostrar el listado
